...
Every day a new word is fetched from an external API alongside with its definition. Users see the definition and part of speech, then guess the word by filling in the letter boxes (that show the length of the word). Additional hints and the give up button to reveal the word are available.
